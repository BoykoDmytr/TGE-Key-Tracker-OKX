...
export function getPollerClient(chain) {
    const existing = clients.get(chain);
    if (existing)
        return existing;
    const urls = rpcsFor(chain);
    if (!urls.length)
        throw new Error(`[poller] no RPCs for chain ${chain}`);
    const client = createPublicClient({
        ...
        urls.map((u) => http(u, { timeout: 8000, retryCount: 0 })), { rank: false }),
    });
    clients.set(chain, client);
    return client;
}
export function rpcCountFor(chain) {
