...
import { classifyDeposit, filterMode, filterAppliesTo } from './filter/spamFilter.js';
...
function logInfo(log, obj, msg) {
    if (log?.info)
        ...
        console.log('[handler]', msg, JSON.stringify(obj));
}
function safeJson(s) {
    try {
        return JSON.parse(s);
...
function networkPretty(chainKey) {
    switch (chainKey) {
...
export async function processSetTimeTx(args, opts) {
    const { chainKey, txHash, to, input } = args;
    const decoded = decodeSetTime(String(input));
    if (!decoded)
        return { sent: false, reason: 'not-settime' };
...
let factoriesLoadedAt = {};
async function ensureFactories(chainKey) {
    const now = Date.now();
    if (now - (factoriesLoadedAt[chainKey] || 0) < 300_000)
        return;
    try {
...
export async function processDepositTx(chainKey, txHash, client, opts) {
    ...
    if (events.length > MAX_FACTORY_EVENTS_PER_TX) {
        ...
        void notifyOwner(`🚨 <b>Absurd factory event count</b>\n\nChain: ${chainKey}\nTx: ${txHash}\n` +
    ...
    let capped = false;
    let withheld = 0;
    const seenInTx = new Set();
    for (const ev of events) {
        try {
            const pairKey = `${ev.token}:${ev.distributor}`;
            if (seenInTx.has(pairKey))
                continue;
            ...
            const landed = sumTransfersTo(receipt, ev.token, ev.distributor);
            ...
            if (meta.degraded) {
                ...
                if (await claimOnce(`metadeg:${chainKey}:${txHash.toLowerCase()}`, 6 * 3600)) {
                    void notifyOwner('\u26A0\uFE0F <b>Token metadata unreadable</b>' + '\n\n' +
                        'Chain: ' + chainKey + '\n' + 'Token: ' + ev.token + '\n' + 'Tx: ' + txHash + '\n\n' +
            ...
            if (threshHuman) {
                ...
                if (threshRaw != null && raw < threshRaw)
                    continue;
            }
    ...
    if (capped) {
        ...
        void notifyOwner(`🚨 <b>Deposit post cap hit</b>\n\nChain: ${chainKey}\nTx: ${txHash}\n` +
            `Events: ${events.length}, posted only ${MAX_DEPOSIT_POSTS_PER_TX}.\n` +
            `All were tracked. Check whether this is a batch or a bug.`);
    }
    if (withheld > 0)
        console.log('[handler] %s %s: %d deposit(s) sent to owner review', chainKey, txHash, withheld);
