...
export const CREATED_TOPIC_NEW = '0xcf9068cf0507f6c18ee38fd73ba24a528f514f0e73ad08229b6db0541071d48d';
// OLD factory 0x000310fa98E36191ec79de241d72C6CA093EAFd3 — data = [token, distributor] (NO amount)
export const CREATED_TOPIC_OLD = '0xe31b7f4b4f3b6042afb5723869d989be921bea013625e326792f25a623ea6c20';
 ...
 */
export function extractDistributorCreated(receipt, isKnownFactory) {
    const events = [];
    const rejected = [];
    for (const log of receipt?.logs ?? []) {
        const addr = String(log?.address ?? '').toLowerCase();
        if (!addr || !isKnownFactory(addr))
            continue; // not our factory -> ignore entirely
 ...
 */
export function sumTransfersTo(receipt, token, distributor) {
    const tok = token.toLowerCase();
    const dst = distributor.toLowerCase();
    let total = 0n;
    for (const log of receipt?.logs ?? []) {
        if (String(log?.address ?? '').toLowerCase() !== tok)
            continue;
        const topics = log?.topics ?? [];
        if (String(topics[0] ?? '').toLowerCase() !== TRANSFER_TOPIC0)
            continue;
        if (topics.length < 3 || typeof topics[2] !== 'string')
            continue;
        if (`0x${topics[2].slice(-40)}`.toLowerCase() !== dst)
            continue;
        try {
