...
const mem = new Map();
export function factoriesFor(chain) {
    let s = mem.get(chain);
    if (!s) {
...
export async function loadFactories(chain) {
    const s = factoriesFor(chain);
    for (const a of ENV_DEFAULT)
        s.add(a);
...
export async function addFactory(chain, address) {
...
export async function listFactories(chain) {
