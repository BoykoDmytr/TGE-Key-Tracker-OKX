...
let working = false;
let lastSentAt = 0;
const sentTimes = []; // timestamps of recent channel sends
...
function prune(now) {
    while (sentTimes.length && now - sentTimes[0] > BREAKER_WINDOW_MS)
        sentTimes.shift();
}
...
async function postMessage(chatId, text, silent, replyMarkup) {
 ...
 */
export async function notifyOwnerWithButtons(text, buttons) {
    try {
...
export async function editOwnerMarkup(messageId, note) {
    try {
        const botToken = requireEnv('TELEGRAM_BOT_TOKEN');
...
function openBreaker(reason) {
    ...
    void notifyOwner(`🚨 <b>Telegram circuit breaker OPEN</b>\n\n` +
        `Reason: ${reason}\n` +
