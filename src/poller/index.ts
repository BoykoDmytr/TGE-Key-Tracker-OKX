...
export type ChainState = 'never-started' | 'ok' | 'lagging' | 'crashed';

export interface ChainStatus {
  cursor: number;
  head: number;
  /** unix seconds of the newest block we have actually scanned; 0 = none yet */
  lastBlockTs: number;
  /** lastBlockTs, or the moment this loop started when nothing has been scanned yet */
  refTs: number;
  /** how far behind chain time we are, in seconds */
  lagSec: number;
  state: ChainState;
  lastError: string;
  updatedAt: number;
}
const STATUS = new Map<string, ChainStatus>();
 ...
 */
export function pollerStatus(): Record<string, ChainStatus & { staleSec: number }> {
  const now = Math.floor(Date.now() / 1000);
  const out: Record<string, ChainStatus & { staleSec: number }> = {};
 ...
 */
async function superviseChainLoop(chain: ChainKey): Promise<void> {
  let attempt = 0;
      ...
      if (nowSec >= crashNextClaimAt) {
        ...
        if (await claimOnce(`pollercrash:${chain}`, LAG_COOLDOWN_SEC)) {
          const sent = await notifyOwner(
            '\u{1F4A5} <b>Poller loop crashed</b>' + '\n\n' +
            'Chain: ' + chain + '\n' + 'Attempt: ' + attempt + '\n' +
...
async function runChainLoop(chain: ChainKey): Promise<void> {
  ...
  if (!FIRST_SEEN.has(chain)) FIRST_SEEN.set(chain, Math.floor(Date.now() / 1000));
  const startedSec = FIRST_SEEN.get(chain) as number;
  ...
  const checkLag = async (): Promise<void> => {
    ...
    if (lagRecoveryDmAt && nowSec >= lagRecoveryDmAt && lagSec < LAG_RECOVER_SEC) {
      lagRecoveryDmAt = (await notifyOwner(
        '\u2705 <b>Poller caught up</b>' + '\n\n' + 'Chain: ' + chain + '\n' + 'Back within ' + lagSec + 's of the head.',
      )) ? 0 : nowSec + LAG_RETRY_SEC;
    }

    if (lagSec > LAG_ALERT_SEC) {
      if (nowSec < lagNextClaimAt) return;
      ...
      const sent = await notifyOwner(
        '\u{1F422} <b>Poller falling behind</b>' + '\n\n' +
        'Chain: ' + chain + '\n' +
      ...
      if (safeHead > cursor) {
        ...
        for (let from = cursor + 1; from <= to; from += CONCURRENCY) {
          ...
          for (let i = 0; i < blocks.length; i++) {
            ...
            if (Number.isFinite(ts)) lastBlockTs = ts;
          }
          if (advanceTo > cursor) {
            cursor = advanceTo;
